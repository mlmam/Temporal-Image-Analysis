...
if isequal(boolcorner,'yes')
    ...
    if scorebool
        ...
        strongCorners = strongCorner.Location;
    ...
    if any(sum(midpoints,2) == sum([mean(midpoints(:,1)),mean(midpoints(:,2))]))
        centermask = sum(midpoints,2) == sum([mean(midpoints(:,1)),mean(midpoints(:,2))]);
        midpoints(centermask,:) = [];

    end
    center = [mean(midpoints(:,1)),mean(midpoints(:,2))];
    scatter(midpoints(:,1),midpoints(:,2));
    scatter(center(1),center(2),'b');
    ...
    cornerMags = mean(cornerMags,3);
    midMags = mean(midMags,3);

    rgb = uint8(cat(3,midMags,cornerMags,centerMags));
    heatFig = figure;
    % imshow(mask);
    % hold on
    hmo = HeatMap(X,'Colormap',rgb);
